...
local function AtGlobalsReset()
	...
	AtGlobal.chainMax    = 6;
	AtGlobal.chainLen    = 2;
	AtGlobal.chainLenInc = 0.5;
	AtGlobal.angleMax    = 6;
	AtGlobal.angleStep   = 6;
...
--AtGlobal.angleStep1  = 1;
  ...
  AtGlobal.reverseDir  = 0;
	...
	AtGlobal.showTrace   = 1;
	AtGlobal.maxBorder   = 0.3;
...
function AutoTractor:update(dt)
	...
	if      self:getIsActiveForInput(false) 
			and AtGlobal.showTrace > 0 
			and ( self.mogliGuiActive 
				 or self.acParameters.enabled 
				 or self.acTurnStage >= 98 ) then	
				 
		if not self.acParameters.enabled and self.acTurnStage < 98 then		
			...
			local a = AutoTractor.getSteeringAngle( self );
			if self.acLastSteeringAngle == nil or math.abs( self.acLastSteeringAngle - a ) > 1E-3 then
				self.acLastSteeringAngle = a;
			...
			for i=1,AtGlobal.chainMax+1 do 
				if math.abs( self.acChain.nodes[i].angle ) > 1E-5 then
					AutoTractor.setChainStatus( self, i, AtStatus.initial );
					self.acChain.nodes[i].angle = 0;
				end
			end
			
			AutoTractor.applyRotation( self,angleFactor,-self.acDimensions.maxSteeringAngle,self.acDimensions.maxSteeringAngle );			
		
			self.acPoints = {};
			if self.acTools ~= nil then
				for i=1,table.getn(self.acTools) do
					local tp = AutoTractor.getSteeringParameterOfTool( self, i, self.acDimensions.maxSteeringAngle, offsetOutside, self.acParameters.leftAreaActive );
					local points ={};
					...
					for i=1,AtGlobal.chainMax+1 do
						...
						local x, z = AutoTractor.getChainPoint( self, i, self.acParameters.leftAreaActive, tp )
	...
	if 1 <= i and i <= AtGlobal.chainMax then
		...
		while i<=i2 and i<=AtGlobal.chainMax do				
			...
			if fieldCheck then
				...
				if fRes then
					...
					if l>1E-3 then
						...
						local f = fStart;
						for j=1,fNum do
							if not AutoTractor.isField( self, xp+f*xd,zp+f*zd,xc+f*xd,zc+f*zd, fStep ) then
								fRes = false
								break;
							end
							f = f + fStep;
			...
			if fRes then
				...
				if bi > 0 and AtGlobal.maxBorder > 0 and toolParam.offset > 0.03 then
					local b2, t2 = AutoTractor.getFruitArea( self, xp, zp, xc, zc, offsetOutside * toolParam.offset, self.acTools[toolParam.i] );						
					if b2 == bi  then
						bi = 0;
						ti = 0;
						bo = bo + b2;
						to = to + t2;
						--print(string.format("ofs=%0.4f, bi=%f, ti=%i, b2=%i, t2=%i",toolParam.offset,bi,ti,b2,t2));
					end
				end
...
--  no reverse allowed	
		...
		local xOffset,_,zOffset = AutoTractor.getRelativeTranslation( tool.steeringAxleNode, tool.refNode );
...
--  normal tool, can be lifted and reverse is possible
		for i=1,table.getn(tool.marker) do
			local xxx,_,zzz = AutoTractor.getRelativeTranslation( self.acChain.refNode, tool.marker[i] );
			if xl < xxx then xl = xxx; zl = zzz; il = i end
			if xr > xxx then xr = xxx; zr = zzz; ir = i end
		end
	...
	-- right	
		...
		r1       = ( 1 + AtGlobal.minMidDist ) * ( r1 + math.max( 0, -z1 ) );
		local a1 = math.atan( self.acDimensions.wheelBase / r1 );
		if lrSwitch then
			angleMin = -maxAngle;
			angleMax = math.min(a1,maxAngle);
...
-- processChain
------------------------------------------------------------------------
function AutoTractor:processChain( toolParams, lrSwitch, angleFactor, offsetOutside )
	...
	for i=indexMin,indexMax do 
		...
		while border < AtGlobal.maxBorder and self.acChain.nodes[i].angle > -AtGlobal.angleMax do				
			doit = false;
			...
			if old1 ~= self.acChain.nodes[i].angle then 
				...
				AutoTractor.applyRotation(self,angleFactor,angleMin,angleMax);				
				border, total  = AutoTractor.getAllChainBorders( self, toolParams, i, indexMax, offsetOutside, lrSwitch, true );
				if border >= AtGlobal.maxBorder then
	...
	if detected and border > AtGlobal.maxBorder then
		detected = false;
	end
	
	if AtGlobal.showTrace > 0 then
		self.acPoints = {};
		for _,tp in pairs(toolParams) do				
			local points ={};
...
-- setChainStraight
------------------------------------------------------------------------
function AutoTractor:setChainStraight( angleFactor, angleMin, angleMax )
	a = self.acLastSteeringAngle;
	for j=1,AtGlobal.chainMax+1 do 
		local old = self.acChain.nodes[j].angle;
		self.acChain.nodes[j].angle = math.min( math.max( -a/angleFactor , -AtGlobal.angleMax ), AtGlobal.angleMax );
		if math.abs( self.acChain.nodes[j].angle - old ) > 1E-5 then
			AutoTractor.setChainStatus( self, j, AtStatus.initial );
		end
	...
	local angle = math.min( math.max( self.acLastSteeringAngle + angleFactor * self.acChain.nodes[1].angle, angleMin ), angleMax );
	return angle;
end
...
-- setChainOutside
------------------------------------------------------------------------
function AutoTractor:setChainOutside( angleFactor, angleMin, angleMax )
	for j=1,AtGlobal.chainMax+1 do 
		local old = self.acChain.nodes[j].angle;
		self.acChain.nodes[j].angle = AtGlobal.angleMax;
		if math.abs( self.acChain.nodes[j].angle - old ) > 1E-5 then
			AutoTractor.setChainStatus( self, j, AtStatus.initial );
		end
	end 
...
-- setChainInside
------------------------------------------------------------------------
function AutoTractor:setChainInside( angleFactor, angleMin, angleMax )
	for j=1,AtGlobal.chainMax+1 do 
		local old = self.acChain.nodes[j].angle;
		self.acChain.nodes[j].angle = -AtGlobal.angleMax;
		if math.abs( self.acChain.nodes[j].angle - old ) > 1E-5 then
...
-- getSteeringAngle
------------------------------------------------------------------------
function AutoTractor:getSteeringAngle()
	steeringAngle = 0;		

	if      self.articulatedAxis ~= nil 
			and self.articulatedAxis.componentJoint ~= nil
			and self.articulatedAxis.componentJoint.jointNode ~= nil then
		steeringAngle = math.min( math.max( -self.lastRotatedTime * self.articulatedAxis.rotSpeed, self.articulatedAxis.rotMin ), self.articulatedAxis.rotMax );
	else
...
--==============================================================		
	...
	self.acLastSteeringAngle = AutoTractor.getSteeringAngle( self );
	...
	detected, angle, border = AutoTractor.processChain( self, toolParams, self.acParameters.leftAreaActive, angleFactor, offsetOutside );					
...
--==============================================================						
	...
	if detected then
		...
		if self.acTurnStage == 99 then
			...
			angle = 0; --AutoTractor:setChainStraight( angleFactor, -angleMax, angleMax );
		end
...
--	if not self.acParameters.leftAreaActive then angle = -angle end
	...
	if self.isEntered then
		AutoTractor.steer( self, dt, angle, detected );
	end
...
-- steer
------------------------------------------------------------------------
function AutoTractor:steer( dt, angle, directSteer )
-- precondition: self.rotatedTime is filled from last steering
	if     angle == 0 then
		targetRotTime = 0
	elseif angle  > 0 then
	...
	if directSteer and AtGlobal.directSteer ~= nil and AtGlobal.directSteer > 0 then
...
-- drive
------------------------------------------------------------------------
function AutoTractor:drive( dt, acceleration, slowAcceleration, slowAngleLimit, allowedToDrive, moveForwards, speedLevel, slowMaxRpmFactor )
...
--==============================================================						
--==============================================================		
	local turnAngle = math.deg(AutoTractor.getTurnAngle(self));
	if self.acParameters.leftAreaActive then
		turnAngle = -turnAngle;
	end;
...
--==============================================================				
	...
	if 0 < self.acTurnStage and self.acTurnStage < 20 then
		detected = false;
	end
	
	AutoTractor.steer( self, dt, angle, detected );
	AutoTractor.drive( self, dt, acceleration, slowAcceleration, slowAngleLimit, allowedToDrive, moveForwards, speedLevel, slowMaxRpmFactor );
...
function AutoTractor:initChain()
	...
	for i=1,AtGlobal.chainMax do
		...
		setTranslation( node2.index, 0,0,AtGlobal.chainLen + ( i-1 ) * AtGlobal.chainLenInc );
...
-- tool with AI support		
		...
		if     object.attacherJoint            == nil
		    or object.attacherJoint.jointType  == nil then
			tool.aiForceTurnNoBackward = false;
		elseif object.attacherJoint.jointType  == Vehicle.JOINTTYPE_TRAILERLOW
      ...
      tool.aiRequiredMaxGrowthState   = fruitDesc.maxPreparingGrowthState; 
		...
		if math.abs( AutoTractor.getRelativeYRotation( self.steeringAxleNode, tool.steeringAxleNode ) ) > 0.6 * math.pi then
		-- wrong rotation ???
